...
void init_memory(u32 mem_high) {
    ...
    initial_page_dir[1023] = ((u32) initial_page_dir - 0xC0000000) | PAGE_FLAG_PRESENT | PAGE_FLAG_WRITE;
...
void mem_change_page_directory(u32* pd) {
    assert_msg(pd > 0xC0000000, "whats up");
    pd = (u32*) (((u32) pd) - 0xC0000000); // calc the physical address
    asm volatile(
        "mov %0, %%eax \n"
        "mov %%eax, %%cr3 \n"
        :: "m"(pd)
...
u32 mem_unmap_page(u32 virt_addr) {
    // kernel_log("pdir=%x mem_unmap_page: %x", mem_get_current_page_directory(), virt_addr);
    ...
    u32* page_dir = REC_PAGEDIR;
    assert_msg(page_dir[pd_index] & PAGE_FLAG_PRESENT, "tried to free page from a non present page table?");

    u32* pt = REC_PAGETABLE(pd_index);
    assert_msg(pt[pt_index] & PAGE_FLAG_PRESENT, "tried to free non present page");
...
u32* mem_alloc_page_dir() {
    for (int i = 0; i < NUM_PAGE_DIRS; i++) {
        if (!page_dir_used[i]) {
            ...
            // recursive mapping
            page_dir[1023] = (((u32) page_dir) - 0xC0000000) | PAGE_FLAG_PRESENT | PAGE_FLAG_WRITE;
            return page_dir;
        }
    }
