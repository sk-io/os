...
void graphics_fill_rect(GraphicsContext* context, s32 x, s32 y, s32 width, s32 height, u32 color) {
    // TODO: optimize?
    for (s32 yi = 0; yi < height; yi++) {
        s32 _y = yi + y;
        if (_y < 0 || _y >= (s32) context->height)
            continue;

        for (s32 xi = 0; xi < width; xi++) {
            s32 _x = xi + x;
            if (_x < 0 || _x >= (s32) context->width)
                continue;
            
            context->framebuffer[_x + _y * context->width] = color;
        }
    }
}
